...
class ExcelExtractor(BaseExtractor):
    ...
    def extract(self) -> pd.DataFrame:
        ...
        logger.info(
            f"Total extracted: {len(combined)} rows from {len(dataframes)} sheets"
        )
    ...
    def extract_streaming(self, chunk_size: int = 1000) -> Iterator[pd.DataFrame]:
        ...
        for sheet in sheets_to_read:
            df = pd.read_excel(
                self.file_path, sheet_name=sheet, header=self.header_row, **self.options
            )
            df["_source_sheet"] = sheet
            yield df
    ...
    def _get_sheets_to_read(self, xl_file: pd.ExcelFile) -> List[str]:
        ...
        if self.sheet_name is None:
            ...
            if self.sheet_name in all_sheets:
                ...
                raise ValueError(
                    f"Sheet '{self.sheet_name}' not found. Available: {all_sheets}"
                )
