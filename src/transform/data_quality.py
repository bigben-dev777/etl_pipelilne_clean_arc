...
class DataQualityScorer:
    ...
    def _score_record(self, row: pd.Series) -> Tuple[int, List[str]]:
        ...
        duplicate_score, duplicate_flags = self._check_duplicate(
    ...
    def _check_address(self, row: pd.Series) -> Tuple[int, List[str]]:
        ...
        present = sum(
            [1 for x in [address1, city, state, zip_code] if x and not pd.isna(x)]
        )
    ...
    def _check_age_info(self, row: pd.Series) -> Tuple[int, List[str]]:
        ...
        has_min_max = (
            min_age is not None
            and not pd.isna(min_age)
            and max_age is not None
            and not pd.isna(max_age)
        )
    ...
    def _check_duplicate(self, is_duplicate: Any) -> Tuple[int, List[str]]:
        ...
        if is_duplicate is True or (
            isinstance(is_duplicate, str) and is_duplicate.lower() == "true"
        ):
